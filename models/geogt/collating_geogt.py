...
def get_laplacian_eigenvectors(adjacency: np.ndarray) -> np.ndarray:
    ...
    L = np.eye(l) - D @ A @ D
    reg_strength = 1e-5
    L += reg_strength * np.eye(l)
    try:
        w, v = np.linalg.eigh(L)
    except np.linalg.LinAlgError:
        print("Eigh failed, using SVD fallback")
...
class GeoGTCollator:
    ...
    def __call__(self, mol_sq: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        eigenvalues = np.zeros((num_mol, max_nodes), dtype=np.float32)
        ...
        for i, mol in enumerate(mol_sq):
            ...
            lap_eigenvectors[i, : num_nodes[i], : num_nodes[i]], eigenvalues[i, : num_nodes[i]]= get_laplacian_eigenvectors(adj)
            adjacency[i, : num_nodes[i], : num_nodes[i]] = adj
            node_type[i, : num_nodes[i]] = np.array(mol["node_type"]) + 1
            in_degree[i, : num_nodes[i]] = np.sum(adj, axis=0, dtype=np.int64) + 1
            out_degree[i, : num_nodes[i]] = np.sum(adj, axis=1, dtype=np.int64) + 1
            if "input_ids" in keys:
                node_input_ids[i, : num_nodes[i]] = np.array(mol["input_ids"]) + 1
            if "conformer" in keys:
                conformer[i, : num_nodes[i]] = mol["conformer"]
            if "node_attr" in keys:
                node_attr[i, : num_nodes[i]] = np.array(mol["node_attr"]) + 1
        res_dic = {
