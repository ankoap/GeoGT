...
class GINConv(MessagePassing):
    ...
    def forward(self, **inputs) -> Any:
        ...
        edge_type = torch.clamp(edge_type, 0, NUM_BOND_TYPE - 1)
        edge_dire_type = torch.clamp(edge_dire_type, 0, NUM_BOND_DIRECTION - 1)
        if node_attr.size(0) == 0:
            raise ValueError("Input batch contains no nodes. Please check the input data.")
...
class GNNEncoder(nn.Module):
    ...
    def forward(self, **inputs):
        ...
        node_type = torch.clamp(node_type, 0, NUM_ATOM_TYPE - 1)
        node_chiral_type = torch.clamp(node_chiral_type, 0, NUM_CHIRALITY_TAG - 1)
