...
class ConformerPredictionHead(nn.Module):
    ...
    def forward(self, conformer: torch.Tensor, hidden_X: torch.Tensor, padding_mask: torch.Tensor = None, compute_loss: bool = True) -> torch.Tensor:
        ...
        temp = mask_hidden_state(conformer_hat, padding_mask)
        conformer_hat = temp
        temp = align_conformer_hat_to_conformer(conformer_hat, conformer)
        conformer_hat = temp
        temp = mask_hidden_state(conformer_hat, padding_mask)
        conformer_hat = temp
    ...
    @staticmethod
    def _compute_conformer_mae(conformer: torch.Tensor, conformer_hat: torch.Tensor, padding_mask: torch.Tensor) -> torch.Tensor:
        mae_elements = F.l1_loss(conformer, conformer_hat, reduction='none')
        mask_3d = padding_mask.unsqueeze(-1).expand_as(conformer)
        sum_mae = (mae_elements * mask_3d).sum()
        num_valid = mask_3d.sum()
        mae = sum_mae / num_valid
        return mae

    @staticmethod
    def _compute_cdist_mae(masked_cdist: torch.Tensor, masked_cdist_hat: torch.Tensor, cdist_mask: torch.Tensor) -> torch.Tensor:
        mae = F.l1_loss(masked_cdist, masked_cdist_hat, reduction="sum") / cdist_mask.sum()
        return mae

    @staticmethod
    def _compute_cdist_mse(masked_cdist: torch.Tensor, masked_cdist_hat: torch.Tensor, cdist_mask: torch.Tensor) -> torch.Tensor:
        mse = F.mse_loss(masked_cdist, masked_cdist_hat, reduction="sum") / cdist_mask.sum()
        return mse
