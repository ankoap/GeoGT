...
def mask_attention_score(attention_score: torch.Tensor, attention_mask: torch.Tensor, mask_pos_value: float = 0.0):
    ...
    attention_mask = attention_mask.repeat_interleave(h, dim=0)
    attention_mask = attention_mask.view(b, h, -1, l)
...
def align_conformer_hat_to_conformer(conformer_hat, conformer):
    ...
    H = torch.matmul(q_centered.transpose(1, 2), p_centered).float()
    U, S, V = torch.svd(H)
    R = torch.matmul(V, U.transpose(1, 2))
    p_rotated = torch.matmul(R, p_centered.transpose(1, 2))
    p_rotated = p_rotated.transpose(1, 2)
    conformer_hat_aligned = p_rotated + q_mean
...
def compute_distance_residual_bias(cdist: torch.Tensor, cdist_mask: torch.Tensor, raw_max: bool = True) -> torch.Tensor:
    ...
    D = cdist * cdist_mask
    if not raw_max:
        D_max, _ = torch.max(D.view(b, -1), dim=-1)
        D = D_max.view(b, 1, 1) - D
        D_max, _ = torch.max(D, dim=-1)
        D = D_max.view(b, l, 1) - D
    D.diagonal(dim1=-2, dim2=-1)[:] = 0
    D = D * cdist_mask
    return D
